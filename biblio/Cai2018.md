...
> **pour**  $$r = 1 ... d$$ **faire** :  
> > **si** $$r > 1$$ **alors** :
> > > Calcul des matrices $$W_{i,w}^{1:r-1}$$
> > > $$\tilde{C}_{i,w} = C_{i,w} - x_{1:r-1}W_{i,w}^{1:r-1}x_{1:r-1}^T$$
> > **sinon** : 
> > > $$\tilde{C}_{i,w} = C_{i,w}$$   
> > Estimation du SCP $$x_r$$ à l'aide de l'algorithme ADMM 
> > Normalisation de $$x_r$$ tel que $$\underset{i}{max} |x_r(i)| =1$$
...
$$ \underset{x_j^1,...x_j^M} \frac{1}{2} \sum_{m = 1}^{M}f(x_j^m) + g(z)\\
\mathrm{s.t.~~} \forall m = 1, \dots, M ~~ x_j^m = z$$

avec $$z\in \mathbb{R}^{P}$$, $$f(x_j) = \| C_{i,w} - x_j(W_{i,w})x_j^T \|_F^2 $$ et $$g(z) = \lambda\|z\|_1 $$
